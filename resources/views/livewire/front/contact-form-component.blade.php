                ...
                <div class="card border-0 shadow-lg form-card p-3">
                    ...
                    <form wire:submit.prevent="submit" class="needs-validation" novalidate>
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control @error('name') is-invalid @enderror" wire:model.defer="name" placeholder="{{ __('messages.your_name') }}" required>
                            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="form-group">
                            <input type="email" class="form-control @error('email') is-invalid @enderror" wire:model.defer="email" placeholder="{{ __('messages.your_email') }}" required>
                            @error('email') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="form-group">
                            <input type="tel" class="form-control @error('phone') is-invalid @enderror" wire:model.defer="phone" placeholder="{{ __('messages.your_phone') }}">
                            @error('phone') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="form-group">
                            <textarea class="form-control @error('message') is-invalid @enderror" wire:model.defer="message" rows="5" placeholder="{{ __('messages.your_message') }}" required></textarea>
                    ...
                    <div class="card-body">
                        ...
                        @foreach($contacts as $item)
                            <div class="d-flex align-items-start mb-3">
                                ...
                                <div>
                                    <div class="font-weight-bold">{{ $item->tr('label') }}</div>
                        ...
                        <div>
                            <div class="text-center mb-2 font-weight-bold">{{ __('messages.follow_us') }}</div>
